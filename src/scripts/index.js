...
$('#enter').tap(function() {
	...
	$.post('http://localhost:8000/skill', function(data) {		
		html += '<div class="jineng content"><p class="headContent"><span>' +
			"技能掌握 " + '</span></p><ul>'
		for(var i = 0; i < data.length; i++) {
			html += '<li><p class="jn_category"><span>' + "类型：" + '</span><span>' + data[i].category + '</span></p><p class="jn_name"><span>' + "获取知识：" + '</span><span>' + data[i].name + '</span></p><p class="jn_time"><span>' + "学习时间：" + '</span><span>' + data[i].time + '</span></p><p class="jn_level"><span>' + "接触程度：" + '</span><span>' + data[i].level + '</span></p></li>';
		}
		html += '</ul></div>';
	});

	$.post('http://localhost:8000/project', function(data) {
		html += '<div class="xiangmu content" style="display:none"><p class="headContent"><span>' + "项目开发" + '</span></p><ul>'
		for(var i = 0; i < data.length; i++) {
			html+= '<li><p class="category"><span>' + "项目类型：" + '</span><span>' + data[i].category + '</span></p><p class="xm_name"><span>' + "项目名称：" + '</span><span>' + data[i].name + '</span></p><p class="url"><span>' + "项目IP：" + '</span><span>' + data[i].url + '</span></p><p class="xm_image"><span>' + "项目图片：" + '</span><span><img src="'+data[i].images+'"></span></p><p class="description"><span>' + "项目描述：" + '</span><span>' + data[i].description + '</span></p><p class="detail"><span>' + "项目详情：" + '</span><span>' + data[i].detail + '</span></p><p class="tech"><span>' + "技术涉及：" + '</span><span>' + data[i].tech + '</span></p></li>';
		}
		html += '</ul></div>';
	});

	$.post('http://localhost:8000/work', function(data) {
		html += '<div class="jingli content" style="display:none"><p class="headContent"><span>'+"工作经历"+'</span></p>'
		for(var i = 0; i < data.length; i++) {
			html+= '<p class="title_out"><span class="title">'+ data[i].category + '</span></p><div class="xuexiao"><div class="jingli_pic"><i class="iconfont icon-location" style="color:gray;"></i><img src="'+data[i].images1+'"><img src="'+data[i].images3+'"><img src="'+data[i].images2+'"></div><div class="jingli_text"><p><span>' + "在读学校：" + '</span><span>' + data[i].category + '</span></p><p><span>' + "在读学校：" + '</span><span>' + data[i].name + '</span></p><p><span>' + "在读学校：" + '</span><span>' + data[i].url + '</span></p><p><span>' + "在读学校：" + '</span><span>' + data[i].time + '</span></p><p><span>' + "在读学校：" + '</span><span>' + data[i].posts + '</span></p><p><span>' + "在读学校：" + '</span><span>' + data[i].projects + '</span></p></div></div>';
		}
		html+= '</ul></div>';
	});

	$.post('http://localhost:8000/me', function(data) {
		html += '<div class="me content" style="display:none"><div class="self_msg"><p class="touxiang"><img src="'+ data[0].images+' "></p><p><span class="name">' + data[0].name + '</span></p><span class="name_miaoshu">' + data[0].name_miaoshu+ '</span></div><div class="school"><p><i class="iconfont icon-xuexiao" style="color:gray;"></i><span class="msg_left">' + "毕业院校" + '</span><span>' + data[0].school + '</span></p><p><span class="msg_left">' + "毕业专业" + '</span><span>' + data[0].profession + '</span></p></div><div class="pingjia"><p><i class="iconfont icon-pingjia" style="color:gray;"></i><span class="msg_left">' + "自我评价" + '</span><span>' + data[0].evaluate + '</span></p></div><div class="lianxifangshi"><p><i class="iconfont icon-tel" style="color:gray;"></i><span class="msg_left">' + "手机号码" + '</span><span>' + data[0].tel + '</span></p><p><i class="iconfont icon-qq" style="color:gray;"></i><span class="msg_left">' + "QQ号码"+ '</span><span>' + data[0].qq + '</span></p><p><img src="' + data[0].erweima + '"></p><P>'+"加入微信，了解更多"+'</P></div>';
